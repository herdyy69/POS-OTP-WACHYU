...
import { productsList } from '@/service/products_service'
...
export default async function Page(params: { searchParams: Promise<SearchParams> }) {
  ...
  const data = await productsList(resolvedSearchParams)

  return (
    <div className='card space-y-4'>
      <div className='space-y-1'>
        <h1 className='plabs-title-medium-18 text-greyscale-9 sm:plabs-title-medium-20'>List Produk</h1>
        <p className='plabs-body-regular-12 text-greyscale-6 sm:plabs-body-regular-14'>
          Kelola produk pada halaman ini.
        </p>
      </div>
      <div className='flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between'>
        <div className='flex items-center gap-3'>
          <TableSearch placeholder='Cari nama produk..' />
        </div>
        <Link href='/products/create' className='btn-green text-xs sm:text-sm'>
