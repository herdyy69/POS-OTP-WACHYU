...
import { InsertProducts } from '@/schemas/products'
import { productsInsert } from '@/service/products_service'
...
export const FormCreate = () => {
  ...
  const onSave = async (data: InsertProducts) => {
    try {
      await productsInsert(data).then(() => {
        toast.success({
          title: 'Success',
      ...
      if (error instanceof Error) {
        ...
        toast.error({
          title: 'Error',
          body: validationErrors[0].message || 'Failed to create product',
        })
      }
    }
  }
  ...
  return (
    <div className='card space-y-6'>
      <div className='space-y-1'>
        <h1 className='plabs-title-medium-20 text-greyscale-9'>Form Tambah Produk</h1>
        <p className='plabs-body-regular-14 text-greyscale-6'>Tambah produk Anda pada halaman ini.</p>
      </div>
      <div className='bg-primary-background plabs-title-medium-16 text-primary-base w-full rounded-2xl px-3 py-2.5'>
        Data Produk
      </div>
      <Form id='create_product' form={form} onSave={onSave} className='space-y-2'>
