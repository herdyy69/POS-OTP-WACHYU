...
import { InsertStores } from '@/schemas/stores'
import { storesInsert } from '@/service/stores_service'
...
export const FormCreate = () => {
  ...
  const onSave = async (data: InsertStores) => {
    try {
      await storesInsert(data).then(() => {
        toast.success({
          title: 'Success',
          body: 'Berhasil menambahkan toko',
        })
        form.reset()
        router.push('/stores')
      })
      ...
      if (error instanceof Error) {
        ...
        toast.error({
          title: 'Error',
          body: validationErrors[0].message || 'Failed to create store',
        })
      }
    }
  }
  ...
  return (
    <div className='card space-y-6'>
      <div className='space-y-1'>
        <h1 className='plabs-title-medium-20 text-greyscale-9'>Form Tambah Toko</h1>
        <p className='plabs-body-regular-14 text-greyscale-6'>Tambah toko Anda pada halaman ini.</p>
      </div>
      <div className='bg-primary-background plabs-title-medium-16 text-primary-base w-full rounded-2xl px-3 py-2.5'>
        Data Toko
      </div>
      <Form id='create_store' form={form} onSave={onSave} className='space-y-2'>
